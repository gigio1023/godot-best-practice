...
5. After edits, run the narrowest meaningful Godot checks: import, syntax, scene load, targeted smoke tests, and visual validation when layout matters.
...
| `references/local/godot-version-and-cli.md` | Starting work, checking version, running validation | Version baseline, CLI commands, import/export/test commands |
...
| `references/local/validation-gates.md` | Before claiming Godot work is complete | Import, syntax, scene-load, smoke, visual, export, and upgrade gates |
...
| `references/local/3d-generation-patterns.md` | Building generated 3D layouts | Semantic layout data, anchors, routes, validation cameras |
...
- For non-trivial scene generation, write a generator script and validate the generated result.
...
- Visual/layout changes: screenshot, recorded run, or explicit viewport validation.
...
- Do not hand-place complex worlds from raw coordinates alone. Use named anchors, source data, and validation cameras.
...
- commands/checks run
- visual/export validation performed, if any
